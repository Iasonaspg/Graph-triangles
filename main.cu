/**********************************************************************
 *
 * main.cu -- main function for the triangle finder algorithm in CUDA
 *
...
int main (int argc, char *argv[]) {
  ...
  if(verboseFlag == 1) {
    ...
    for(int i = 0; i < NQ ; i++){
      for (int d=0; d<DIM; d++)
	printf("%1.4f ", Q[i+d*NQ]);
      printf("\n");
    }
    printf(" ======C vector====== \n");
    for(int i = 0; i < NC ; i++){
      for (int d=0; d<DIM; d++)
	printf("%1.4f ", C[i+d*NC]);
      printf("\n");
    }
  }
  ...
  cuNearestNeighbor<<<numberOfBlocks, threadsPerBlock>>>
    (d_C,NC,d_S,d_Q,NQ,d_QBoxIdToCheck,d,d_neighbor,d_checkOutside);
